...
"Tiki will use display CSS-styled (Suckerfish) vertical or horizontal menus" => "CSS 메뉴 (서커피시)",
...
"If enabled users (with permission) can see the sortable, searchable list of Wiki pages (tiki-lastchanges.php) organized by last updated date. Use the Configuration area to specify which items to display" => "최종 변경된 항목들의 목록 표시",
"Orphan pages" => "고아 페이지",
...
"This is for adding additional content such at META or LINK tags, in the HEAD section of HTML files. Smarty filters can be used here" => "사용자 지정 &lt;메타&gt; 혹은 &lt;링크&gt; 태그를 포함하기 위하여 사용",
...
"Undo" => "복구",
"Footnotes" => "각주",
"If enabled logged users can write personal footnotes when editing a page. The footnotes will be displayed only to the user who edited them immediately below the Wiki page. This can be used as a good way to let users make personal notes and observations about pages" => "저자에게만 보이는 페이지에 대한 개인 메모 만들기",
...
"Enabling the option will add a Discuss button on every Wiki page that links to a forum thread dedicated to discussion the particular page" => "각 페이지에 게시판 토픽 링크 포함",
"View source" => "소스 보기",
"Users (with permission) can see the Wiki syntax of the Wiki page" => "위키 페이지의 소소를 보기 사용",
...
"For use with the !!- or !!+ syntax. Checking will display the showhide switch at the left of the heading, otherwise it displays below" => "제목 앞에 표시되는 표시/숨김 아이콘 표시",
...
"Invite users" => "사용자 초대",
"Allow users to invite new users by mail to register at this site.  (by sending an email)" => "사용자들이 새 사용자에게 이 티키로 등록하도록 이메일을 보내 초대하는 것을 허용함",
...
"Tiki will include a button on the registration form that will automatically generate a very secure password for the user" => "\"비밀번호 생성\" 선택사항을 등록 서식에 포함",
...
"This will allow users to register, using the webform. The Login module will include a Register link. If disabled, the admin will have to create new users manually on the Admin Users page. " => "사용자 등록 허용",
...
"Additional CSS rules can be entered here and will apply to all pages, or the page’s CSS id can be used to limit the scope of the rule (check the page’s HTML source to find the body tag of the particular page.)" => "모든 페이지 내부에 CSS 인라인의 사용자 지정 블럭을 포함합니다",
...
"Never delete versions younger than (? days)" => "절대 (? 일) 보다 어린 버전을 삭제하지 않습니다. 기본=1",
...
"Constrains the width of the content area of the site" => "사이트 표시 넓이를 제한함 (기본: 990px)",
...
"Use OpenPGP PGP/MIME compliant encrypted email messaging. All email-messaging/notifications/newsletters are sent as PGP/MIME-encrypted messages, signed with the signer-key, and are completely 100% opaque to outsiders. All user accounts need to be properly configured into gnupg keyring with public-keys related to their tiki-account-related email-addresses" => "OpenPGP PGP/MIME 에 준하는 암호화 이메일 메시징 사용 (기본은 'n\\",
...
"Password must contain at least one lowercase alphabetical character like \"a\" and one uppercase character like \"A\". Use this option to force users to select stronger passwords" => "비밀번호는 a 와 같은 소문자와 A 와 같은 대문자를 반드시 최소 한 개씩 포함해야만 함",
"Require special characters" => "특수 캐릭터 요구",
"Password must contain at least one special character in lower case like <b>\" / \$ % ? & * ( ) _ + .</b> Use this option to force users to select stronger passwords" => "비밀번호는 최소 다음과 같은 특수 문자를 포함해야합니다 \" / \$ % ? & * ( ) _ + ...",
...
"Poll ordering algorithm" => "투표 정렬순서 알고리듬 (기본=점수)",
...
"Use proxy" => "프록시 사용",
"Specify if Tiki requires a proxy to access the internet. If enabled, you can specify your proxy Host name (either with or without the http:// prefix), Port settings, Username, and Password" => "프록시 사용",
...
"Tiki will display the page name in small text at the top of each page (above the page’s content)" => "페이지 이름을 페이지 내부 대신 페이지 위에 표시",
...
"Specify the location of the edit buttons (such as Save, Preview, and Cancel) when editing wiki pages" => "버튼: 저장, 미리보기, 취소, ..",
